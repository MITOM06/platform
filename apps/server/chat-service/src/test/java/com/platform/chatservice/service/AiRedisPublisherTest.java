...
/**
 * Verifies the RabbitMQ inject side of the end-to-end trace: the publisher must (a) start an
 * ai.request.publish span, (b) run the MessagePostProcessor that injects the W3C traceparent into
 * the AMQP message headers, and (c) close the scope then end the span.
 */
  ...
  void publishAiRequest_injectsTraceparentIntoAmqpHeadersAndEndsSpan() {
    ...
    doAnswer(
            inv -> {
              ...
              setter.set(
                  props, "traceparent", "00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-01");
              return null;
            })
        .when(propagator)
