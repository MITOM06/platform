 ...
 *
 * Uses Testcontainers to spin up a real MongoDB 7 instance so that the actual
 * Spring Data query methods and compound indexes are exercised — bugs in query
 * derivation or index definitions are caught here but would be invisible in the
 * mock-based unit tests in {@link MessageServiceTest}.
 *
 * Annotated with {@code @DataMongoTest} (MongoDB slice) so the full application
 * context (RabbitMQ, Redis, JWT secret, Firebase) is NOT started. Non-MongoDB
 * collaborators are supplied as hand-wired mocks.
 */
@DataMongoTest
@Testcontainers
...
class MessageServicePaginationTest {
    ...
    void setUp() {
        ...
        messageService = new MessageService(
                ...
                helper, messageMapper, aiMessageService);
     ...
     * {@code getMessages(convId, null, 2)} must return exactly 2 messages, NEWEST
     * first (msg-it-4, msg-it-3), and {@code hasNext == true} because there are 3
     * more older messages.
     */
    @Test
    ...
    /**
     * TEST 2 — Cursor page:
     * Using the last message from page 1 (msg-it-3) as cursor, the next page must
     * return msg-it-2 and msg-it-1 (strictly older, newest-first within the page),
     * with NO overlap with the first page.
     */
    @Test
    void cursorPage_returnsNextTwoOlderMessages_noOverlapWithFirstPage() {
        ...
        assertThat(page2.content()).extracting(MessageResponse::id)
                .doesNotContain("msg-it-4", "msg-it-3");
    }

    /**
     * TEST 3 — Last page / hasMore == false:
     * After consuming page1 (msg-4, msg-3) and page2 (msg-2, msg-1), the final
     * page using cursor = msg-it-1 must return only msg-it-0 and {@code hasNext ==
    ...
    void lastPage_hasMoreIsFalse_andRemainingCountIsCorrect() {
        ...
        PageResponse<MessageResponse> lastPage = messageService.getMessages(USER_ID, CONV_ID, cursor, 2);
     ...
     * Messages for a different conversationId must NOT appear in paginated results
    ...
    void conversationIsolation_differentConvMessages_notReturned() {
        ...
        conversationRepository.save(Conversation.builder()
                ...
                .type(Conversation.TYPE_DIRECT)
                .build());
        Message otherMsg = Message.builder()
                .id("msg-other-0")
        ...
        assertThat(page.content()).extracting(MessageResponse::conversationId)
                .containsOnly(CONV_ID);
        assertThat(page.content()).extracting(MessageResponse::id)
                .doesNotContain("msg-other-0");
    }
}
