...
/**
 * Background sweep for disappearing-messages. Extracted from {@code MessageService}
 * to keep that class within the clean-code line limit. Behavior is unchanged:
 * after a sweep removes messages, the conversation's lastMessage/lastMessageAt
 * are refreshed from the newest remaining message.
 */
@Service
...
public class MessageSweepService {
    ...
    private void refreshLastMessage(String conversationId) {
        Query latestQuery = new Query(Criteria.where("conversationId").is(conversationId))
