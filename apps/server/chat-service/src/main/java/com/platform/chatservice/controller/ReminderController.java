...
public class ReminderController {
  ...
  public List<ReminderResponse> getReminders(Authentication auth) {
    ...
    return reminderRepository.findByUserIdAndDoneFalseOrderByRemindAtAsc(userId).stream()
        .map(this::toResponse)
        .toList();
  }

  @PatchMapping("/{id}/done")
  public ResponseEntity<ReminderResponse> markDone(@PathVariable String id, Authentication auth) {
    String userId = auth.getName();
    Reminder reminder =
        reminderRepository
  ...
  @DeleteMapping("/{id}")
  public ResponseEntity<Void> deleteReminder(@PathVariable String id, Authentication auth) {
    String userId = auth.getName();
    reminderRepository
        .findByIdAndUserId(id, userId)
        .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
