 ...
 *
 * <p>Injects W3C {@code traceparent} into the AMQP message headers so that
 * the ai-service can extract the propagated context and continue the same
 * distributed trace (chat-service → RabbitMQ → ai-service).
 */
@Service
        ...
        try (Tracer.SpanInScope ws = tracer.withSpan(span)) {
            rabbitTemplate.convertAndSend(
                ...
                message -> {
                    ...
                    propagator.inject(span.context(), message.getMessageProperties(),
