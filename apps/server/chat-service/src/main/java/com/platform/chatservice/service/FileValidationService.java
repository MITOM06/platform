...
/**
 * Validates uploads via magic-bytes signature check and per-type size caps.
 * Call {@link #validate} before storing any uploaded file.
 */
@Service
@Slf4j
public class FileValidationService {
    ...
    private void validateMagicBytes(MultipartFile file, String contentType) throws IOException {
        ...
        } else if ("application/x-rar-compressed".equals(lower) || "application/vnd.rar".equals(lower)) {
    ...
    private boolean isAllowedImage(byte[] h) {
        return isJpeg(h) || isPng(h) || isGif(h) || isWebP(h) || isBmp(h)
                || isFtypBox(h)         // HEIC / HEIF use ISO Base Media box
                || isSvgOrXml(h);       // SVG is text-based
    }

    private boolean isJpeg(byte[] h)  { return startsWith(h, 0xFF, 0xD8, 0xFF); }
    private boolean isPng(byte[] h)   { return startsWith(h, 0x89, 0x50, 0x4E, 0x47); }
    private boolean isGif(byte[] h)   { return startsWith(h, 0x47, 0x49, 0x46, 0x38); }
    ...
    private boolean isBmp(byte[] h)   { return startsWith(h, 0x42, 0x4D); }
    ...
    private boolean isAvi(byte[] h) {
        return startsWith(h, 0x52, 0x49, 0x46, 0x46)  // RIFF
                && h.length >= 12
                && h[8] == 0x41 && h[9] == 0x56 && h[10] == 0x49 && h[11] == 0x20; // AVI<space>
    }

    // --- audio signatures ---------------------------------------------------

    private boolean isAllowedAudio(byte[] h) {
        return isMpegAudio(h) || isId3(h) || isOgg(h) || isWav(h)
                || isFlac(h) || isFtypBox(h); // M4A uses ISO Base Media too
    }
    ...
    private boolean isId3(byte[] h)  { return startsWith(h, 0x49, 0x44, 0x33); } // ID3

    private boolean isOgg(byte[] h)  { return startsWith(h, 0x4F, 0x67, 0x67, 0x53); } // OggS
    ...
    private boolean isFlac(byte[] h) { return startsWith(h, 0x66, 0x4C, 0x61, 0x43); } // fLaC
    ...
    private boolean isEbml(byte[] h) { return startsWith(h, 0x1A, 0x45, 0xDF, 0xA3); }
