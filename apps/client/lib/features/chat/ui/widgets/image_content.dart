...
// `openExternally` / `downloadMedia` moved to media_actions.dart. Re-exported
// so existing `image_content.dart` importers (file_content, link_preview_card)
// keep working unchanged.
export 'media_actions.dart' show openExternally, downloadMedia;
...
class _SingleImageTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () => openImageGallery(context, allUrls, index),
      child: image,
    );
  }
}
  ...
  Widget _overlayTile(int index, double width, double height, int extras) {
    return Builder(
      builder: (ctx) => GestureDetector(
        onTap: () => openImageGallery(ctx, urls, index),
        child: Stack(
          children: [
            CachedNetworkImage(
