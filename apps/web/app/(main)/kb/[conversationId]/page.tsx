...
function StatusChip({ doc, labels }: { doc: KbDocument; labels: { processing: string; ready: string; error: string } }) {
...
function DocumentTile({
  ...
  doc: KbDocument
  onDelete: () => void
  statusLabels: { processing: string; ready: string; error: string }
    ...
    <div className="group rounded-xl border bg-card p-4 transition-all hover:shadow-lg hover:border-pon-cyan/30 relative overflow-hidden flex items-center gap-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3">
          <StatusChip doc={doc} labels={statusLabels} />
          {doc.status === 'done' && (
            <span className="text-[11px] text-muted-foreground">
              {chunksLabel(doc.chunkCount)}
            </span>
          )}
        </div>
      </div>
            ...
            <div className="space-y-3">
              {docs.map((doc) => (
                <DocumentTile
                  ...
                  chunksLabel={(count) => t('chunks', { count })}
                />
              ))}
            </div>
          )}
