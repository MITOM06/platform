...
| Layer 3 — remaining batches (AI / admin / remainder) | spacing + typography + layout per screen, ad-hoc text-alpha audit. **Known leftovers:** violet `rgba(180,127,255,…)` in `ai-hub` (batch 4) and `admin/usage-dashboard` (batch 5); violet→teal gradient in `assistant_sheen_avatar.dart` + `AssistantEntry.tsx` + `AssistantPreviewAvatar.tsx` (batch 4, fix together for parity) | Not written yet — see the direction doc §3, P2's "Còn lại cho Layer 3", and L3-pre's "Cố ý KHÔNG làm". **Note:** dropping the no-op `PonCard`/`PonButton`/`PonTextField` param *declarations* needs a final pass after all 6 batches (~70 call sites still pass them) |
