...
| Layer 3 batch 2b — Chat core, long tail | ~180 remaining `Colors.white/black` in ~30 chrome widgets + 28 oversized radii. Media/call/viewer surfaces keep white **by design** — list in 2a's "Cố ý KHÔNG làm" | Not written yet |
| Layer 3 — remaining batches (settings / AI / admin / remainder) | spacing + typography + layout per screen, ad-hoc text-alpha audit | Not written yet — see the direction doc §3, P2's "Còn lại cho Layer 3", and L3-pre's "Cố ý KHÔNG làm". **Note:** dropping the no-op `PonCard`/`PonButton`/`PonTextField` param *declarations* needs a final pass after all 6 batches (~70 call sites still pass them) |
