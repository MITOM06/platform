...
| Layer 3 — remaining batches (admin / remainder) | spacing + typography + layout per screen, ad-hoc text-alpha audit. **Known leftover:** violet `rgba(180,127,255,…)` in `components/admin/usage-dashboard.tsx` (batch 5) | Not written yet |
| Layer 3 — final pass (after all batches) | (a) delete the no-op `PonCard`/`PonButton`/`PonTextField` param *declarations* once no call site passes them; (b) app-wide `Colors.redAccent` → `colorScheme.error` sweep (79 sites — `redAccent` is `#FF5252` while the palette's light destructive is `#B3261E`) | Not written yet | — see the direction doc §3, P2's "Còn lại cho Layer 3", and L3-pre's "Cố ý KHÔNG làm". **Note:** dropping the no-op `PonCard`/`PonButton`/`PonTextField` param *declarations* needs a final pass after all 6 batches (~70 call sites still pass them) |
