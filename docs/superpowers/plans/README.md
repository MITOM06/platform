...
| Layer 3 batch 6 — Remainder | friends, reminders, help, integrations, skills, notifications, home, explore, blocked, archived, legal, shared-media. Flutter: ~74 literals, 2 gradients, 7 `darkSurface`, 11 radii, 18 no-op params. **Web side is already clean** | Not written yet |
| Layer 3 — final pass (after batch 6) | (a) delete the no-op `PonCard`/`PonButton`/`PonTextField` param *declarations* once no call site passes them (35 call sites left, all in batch-6 territory); (b) app-wide `Colors.redAccent` → `colorScheme.error` sweep (64 sites — `redAccent` is `#FF5252` while the palette's light destructive is `#B3261E`); (c) check the backend default for `workspace.primaryColor` (batch 5 fixed only the client fallbacks) | Not written yet |
